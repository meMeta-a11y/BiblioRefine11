...
import pandas as pd
import numpy as np
import json, os, re, time, hashlib, datetime, subprocess
...
# Parsing optional libs (graceful)
try:
    import bibtexparser
except Exception:
    bibtexparser = None
...
# MASTER mapping for common WoS/Scopus column names
MASTER_MAP = {
...
# -------------------------
# Friendly dependency summary to show at start
# -------------------------
def dependency_report():
    deps = {
        ...
        'bibtexparser (parse .bib)': bibtexparser is not None,
        'rispy (parse .ris)': rispy is not None
    }
    return deps

# -------------------------
# Helper functions: standardize & normalize
# -------------------------
...
# Parsing extras
# -------------------------
def parse_bib_bytes(b):
    if not bibtexparser: return pd.DataFrame()
    try:
        ...
        for e in bib_db.entries:
            r = {
                'TI': e.get('title'),
                'AU': '; '.join([a.strip() for a in re.split(r'\s+and\s+', e.get('author',''))]) if e.get('author') else pd.NA,
                'AF': None,
                'PY': e.get('year'),
                'SO': e.get('journal') or e.get('booktitle'),
                'DI': e.get('doi') or e.get('url'),
                'AB': e.get('abstract'),
                'DE': e.get('keywords')
            }
            r['AF'] = r['AU']
...
def parse_ris_bytes(b):
    if not rispy: return pd.DataFrame()
    try:
...
def parse_openalex_json_bytes(b):
    try:
        txt = b.decode('utf-8', errors='ignore')
        data = json.loads(txt)
        rows=[]
...
# -------------------------
# Tokenization and keyword combining
# -------------------------
...
# -------------------------
# Normalization and graph building
# -------------------------
...
# -------------------------
# Clustering functions with fallback and friendly messages
# -------------------------
def cluster_with_leiden(G:nx.Graph, seed:int=DEFAULT_SEED) -> Dict[int,int]:
    # convert to igraph
    mapping = {n:i for i,n in enumerate(G.nodes())}
    rev = {i:n for n,i in mapping.items()}
    if not LEIDEN_AVAILABLE:
        raise RuntimeError("Leiden libs are not available in this environment.")
    g = ig.Graph()
...
def cluster_with_louvain(G:nx.Graph, seed:int=DEFAULT_SEED) -> Dict[int,int]:
    if community_louvain is None:
        # fallback trivial
        return {n: 0 for n in G.nodes()}
    return community_louvain.best_partition(G, weight='weight', random_state=seed)

def run_clustering(G:nx.Graph, prefer_leiden:bool, seed:int=DEFAULT_SEED):
    if prefer_leiden and LEIDEN_AVAILABLE:
        return cluster_with_leiden(G, seed=seed)
    else:
        if prefer_leiden and not LEIDEN_AVAILABLE:
            st.warning("Leiden not available — using Louvain instead. To enable Leiden, install python-igraph and leidenalg on Python 3.10/3.11.")
        return cluster_with_louvain(G, seed=seed)
...
def windowed_clustering(df:pd.DataFrame, window_size=DEFAULT_WINDOW, stride=DEFAULT_STRIDE, co_word_params:Dict[str,Any]=None, progress_cb=None):
...
# -------------------------
# Timeline builder and alluvial plot
# -------------------------
        ...
        for a_id, a_terms in ca.items():
            for b_id, b_terms in cb.items():
                ...
                if j >= overlap_threshold:
                    sname = f"{a}|C{a_id}"; tname = f"{b}|C{b_id}"
                    if sname in node_index and tname in node_index:
                        links.append({'source': sname, 'target': tname, 'value': float(j)})
...
# VOSviewer exports
# -------------------------
def build_vos_files(G:nx.Graph, partition:Dict[int,int]):
    # nodes: "Id\tLabel"
    nodes_txt = "Id\tLabel\n" + "\n".join([f"{n}\t{G.nodes[n]['term']}" for n in G.nodes()])
    # edges: "Source\tTarget\tWeight"
    edges_txt = "Source\tTarget\tWeight\n" + "\n".join([f"{u}\t{v}\t{d.get('weight',1.0)}" for u,v,d in G.edges(data=True)])
    # clusters .clu : first line number of nodes? VOSviewer sometimes expects header: "COCluster_Version 1" etc.
    # We'll provide simple list per node index (sorted by node id)
    clu_txt = "\n".join([str(partition.get(n,0)) for n in sorted(G.nodes())])
    return nodes_txt, edges_txt, clu_txt

# -------------------------
# Progress callback factory for nice UI
# -------------------------
...
# Streamlit UI
# -------------------------
st.title("📚 BiblioRefine — Optimized (≤10k) with friendly errors & Leiden→Louvain fallback")

# show dependency report
deps = dependency_report()
st.sidebar.header("Dependency status")
for k,v in deps.items():
    st.sidebar.write(f"{k}: {'✅' if v else '⚠️'}")

st.sidebar.markdown("If Leiden is not available (⚠️), the app will use Louvain automatically.")
...
# file upload UI
st.header("Step 1 — Upload & prepare data (WoS .xlsx, Scopus .csv, optional .bib/.ris/.xml/.json)")
...
if st.button("Load & prepare (limit to 10k)"):
    ...
    if wos_file:
        try:
            ...
            dfw = standardize_df(dfw, 'WoSCC'); dfs.append(dfw)
            st.success(f"Loaded WoS — {dfw.shape[0]} rows")
        except Exception as e:
            st.error(f"Could not read WoS file: {e}")
    if scopus_file:
        try:
            dfsc = pd.read_csv(scopus_file, low_memory=False)
            dfsc = standardize_df(dfsc, 'Scopus'); dfs.append(dfsc)
            st.success(f"Loaded Scopus — {dfsc.shape[0]} rows")
        except Exception as e:
            st.error(f"Could not read Scopus file: {e}")
    parsed = []
    if extra_files:
        for f in extra_files:
            ...
            try:
                ...
                if not dfx.empty:
                    parsed.append(dfx)
                    st.info(f"Parsed {f.name}: {dfx.shape[0]} rows")
            except Exception as e:
                st.warning(f"Failed to parse {f.name}: {e}")
    if parsed:
        dfs.append(pd.concat(parsed, ignore_index=True, sort=False))
    if not dfs:
        st.error("No files loaded. Please upload WoS/Scopus or extras.")
    else:
        ...
        # sample to MAX_RECORDS if needed
        if len(merged) > MAX_RECORDS:
            st.warning(f"Dataset has {len(merged)} records — sampling {MAX_RECORDS} for performance.")
...
# Run analysis (co-word, clustering, evolution)
if st.button("Run analysis (co-word, clustering, thematic evolution)"):
    if 'merged' not in st.session_state:
        st.error("Please load & prepare dataset first.")
    else:
        merged = st.session_state['merged']
        st.info("Preparing keywords (DE + ID) and tokenizing...")
        kw_lists = []
        ...
        st.write(f"Using normalization: {norm_label}")

        # graph build
        st.info("Building NetworkX graph")
        G = build_graph_from_similarity(S, terms, threshold=edge_threshold, top_k=None)
        st.write(f"Graph created: nodes={G.number_of_nodes()}, edges={G.number_of_edges()}")
        ...
        try:
            ...
            st.warning(f"Clustering error or Leiden not available: {e}. Falling back to Louvain.")
            partition = cluster_with_louvain(G, seed=seed)
        ...
        st.info("Running windowed clustering (thematic evolution)...")
        ...
        window_results = windowed_clustering(merged, window_size=window_size, stride=stride, co_word_params=co_params, progress_cb=progress_callback_factory(progress_bar, status))
        window_results = {k:v for k,v in window_results.items() if v is not None}
        if not window_results:
            st.warning("No valid windowed results (check PY/year fields).")
        else:
        ...
        # write run metadata
        metadata = {'timestamp': datetime.datetime.utcnow().isoformat()+'Z', 'params': {'vocab_size':vocab_size,'min_df':min_df,'edge_threshold':edge_threshold,'use_association':use_association,'use_leiden':prefer_leiden,'seed':seed,'window_size':window_size,'stride':stride,'overlap_thresh':overlap_thresh}}
        try:
        ...
        st.success("Run metadata saved to run_metadata.json")

# -------------------------
# Exports: CSV, VOSviewer, JSON, ZIP
# -------------------------
if 'global_graph' in st.session_state:
    ...
    # term->cluster CSV
    term_cluster_rows = [{'term': G.nodes[n]['term'], 'node_idx': n, 'cluster': int(partition.get(n,0))} for n in G.nodes()]
    df_term_cluster = pd.DataFrame(term_cluster_rows)
    ...
    # VOSviewer files
    vos_nodes_txt, vos_edges_txt, vos_clu_txt = build_vos_files(G, partition)
    ...
    # ZIP package
    if st.button("Prepare ZIP with all outputs"):
        buf = BytesIO()
        with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            try:
                ...
                pass
        buf.seek(0)
        st.download_button("Download ZIP package", buf, "biblio_results.zip")

st.caption("Notes: If Leiden isn't available on your environment (common on newer Python versions), the app will automatically fall back to Louvain. To enable Leiden reliably, run the app on Python 3.10/3.11 and install python-igraph + leidenalg.")
