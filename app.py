...
# Upload & Merge 
# -----------------------------
st.header("Upload and Prepare Data")
...
if wos_file and scopus_file:
    ...
    # -----------------------------
    # Recover Missing DOIs from PubMed
    # -----------------------------
    st.header("Recover Missing DOIs from PubMed")
...
st.header("Enrich with OpenAlex")
...
if st.sidebar.button("Show Data Quality Report"):
    if dfq is None:
        st.sidebar.warning("Run OpenAlex enrichment first")
    else:
        FIELDS = ['TI','AU','AF','SO','PY','DE','AB','C1','FX','CR','NR']
        rows = []
        total = len(dfq)
...
# -----------------------------
# Analysis - Co-word, Bibliographic coupling, Co-citation
# -----------------------------
st.header("Analysis")
...
def build_doc_ref_matrix_from_CR(cr_series: pd.Series):
    if cr_series is None or cr_series.empty:
        return None, [], []
    docs = []
    ref_index = {}
    rows = []
    for i, cr in cr_series.fillna("").items():  # ← fixed here
